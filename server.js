...
// Проверка на слишком похожие события
...
1. Текст события должен быть от 4 до 6 предложений
2. Не ставь кавычки в начале текста
3. После текста события поставь пустую строку
4. Затем напиши "Последствия:" с новой строки
5. Каждое последствие с новой строки, начинается с дефиса и пробела (-)
6. Между последствиями не ставь пустые строки

ПРИМЕР ПРАВИЛЬНОГО ФОРМАТА:
Группа отправилась на разведку к заброшенной заправке. Внутри оказалось темно и сыро, пол был усыпан битым стеклом. Внезапно из темноты выскочила одичавшая собака и укусила Марию за ногу. Собаку отогнали, но Мария поранилась о стекло.

Последствия:
- Мария: рваная рана ноги (средняя)
- Бункер: минус 1 рулон бинтов
...
НАПИШИ ОДНО СОБЫТИЕ (${eventType}) В ТАКОМ ФОРМАТЕ (4-6 предложений, без кавычек):
...
async function generateValidatedEvent(game) {
  ...
  const category = pickCategoryAvoiding(new Set());
  const storyPrompt = generateEventPromptWithCategory(game, category, "");
  
  let storyResult = null;
  for (const model of STORY_MODELS) {
    try {
      storyResult = await callModelWithTimeout(model, storyPrompt, 20000);
      console.log(`✅ Событие сгенерировано моделью ${model}`);
      break;
    } catch (error) {
...
app.post('/api/generate-event', async (req, res) => {
  try {
    ...
    const parts = validatedEvent.split(/\n\s*Последствия:/i);
