...
function normalizeTextForSimilarity(text) {
  return (text || "")
    .toLowerCase()
    // выкидываем блок последствий, чтобы сравнивать именно сюжет
    .replace(/последствия:\s*[\s\S]*$/i, "")
    // убираем "тип события" если вдруг остался
    .replace(/тип события:\s*.*$/gim, "")
    // убираем пунктуацию/лишние символы
    .replace(/[^\p{L}\p{N}\s]+/gu, " ")
    .replace(/\s+/g, " ")
    .trim();
}

// 2) Jaccard по биграммам слов (устойчивее, чем по словам)
...
// 3) Проверка "слишком похоже" на последние N событий
function isTooSimilarToRecent(candidateText, game, options = {}) {
  const {
    recentCount = 10,     // сколько последних событий сравнивать
    threshold = 0.35      // порог похожести (0..1). 0.30-0.45 обычно норм
  } = options;
...
// 4) Категории (как у тебя) + функция выбрать категорию НЕ из последних N
const EVENT_CATEGORIES = [
  'Группа замечает дым/огонь неподалеку и идет проверить',
  'Слышны звуки борьбы или крики неподалеку',
  'Находят свежие следы людей или животных',
  'Замечают сигнальные огни/ракеты вдалеке',
  'Обнаруживают заброшенный лагерь с остатками припасов',
  'Встречают других выживших (раненых, враждебных, дружелюбных)',
  'Находят транспортное средство (машина, велосипед)',
  'Замечают, что за ними кто-то наблюдает',
  'Натыкаются на ловушку, поставленную другими людьми',
  'Обнаруживают схрон с припасами, но он заминирован или охраняется',
  'Кто-то из группы проваливается в яму или получает травму при передвижении',
  'Замечают беспилотник или другую технику',
  'Находят труп и при нем полезные вещи',
  'Замечают, что пропали некоторые вещи (возможно, кража)',
  'Сталкиваются с дикими животными'
];

// Если хочешь — можно сохранять category в событии при записи (идеально).
// Но даже без этого можно грубо извлечь категорию из текста события по ключевым словам.
// Здесь упрощённо: мы будем просто избегать категорий, которые уже были выбранными в рамках текущих попыток.
function pickCategoryAvoiding(avoidSet = new Set()) {
  const pool = EVENT_CATEGORIES.filter(c => !avoidSet.has(c));
  const list = pool.length > 0 ? pool : EVENT_CATEGORIES;
...
function generateEventPromptWithCategory(game, category, extraUniqInstructions = "") {
  ...
  if (recentEvents.length > 0) {
    ...
    recentEventsText = 'Это первое событие в игре.';
  }
  ...
  // Случайно выбираем тип события (60% негатив, 40% позитив)
  const eventType = Math.random() < 0.6 ? 'НЕГАТИВНОЕ' : 'ПОЗИТИВНОЕ';

  return `Ты генератор событий для игры "Бункер".

ВАЖНЕЙШЕЕ ПРАВИЛО: Каждое событие ДОЛЖНО иметь конкретные последствия из списка разрешенных предметов, болезней или изменений ресурсов бункера.

ТИП СОБЫТИЯ: ${eventType}

ЕСЛИ НЕГАТИВНОЕ (60%):
- Нападение животных/людей/мутантов
- Несчастный случай (падение, обвал, пожар, взрыв)
- Природная опасность (буря, мороз, радиация)
- Болезнь или заражение
- Поломка снаряжения

Последствия негативного события ВСЕГДА:
- Травма/болезнь/фобия у 1-2 игроков
- Потеря предметов инвентаря
- Уменьшение запасов бункера
- Поломка оборудования

ЕСЛИ ПОЗИТИВНОЕ (40%):
- Находка припасов в разрушенных зданиях
- Обнаружение схрона/тайника
- Встреча с дружелюбными выжившими (обмен)
- Находка полезных инструментов

Последствия позитивного события ВСЕГДА:
- Добавление предметов в инвентарь игроков
- Увеличение запасов бункера на конкретное количество месяцев (1-12)
- Находка инструментов для бункера

ПРАВИЛА НАПИСАНИЯ:
1. Пиши коротко и просто, 2-4 предложения
2. Сразу описывай что случилось и чем закончилось
3. Каждое последствие с новой строки и с дефиса
4. Указывай КОНКРЕТНЫЕ предметы из списка разрешенного инвентаря
5. Для еды указывай точное количество месяцев (+2 месяца еды)
6. Для болезней указывай степень (легкая, средняя, тяжелая)
...
РАЗРЕШЕННЫЙ ИНВЕНТАРЬ (ТОЛЬКО ЭТИ ПРЕДМЕТЫ):
${allowedInventory}

РАЗРЕШЕННЫЕ БОЛЕЗНИ (ТОЛЬКО ЭТИ):
- Перелом, Вывих, Растяжение, Рваная рана, Сотрясение мозга
- Пневмония, Ангина, ОРВИ, Отравление
- Ожог, Обморожение, Радиационное поражение

${recentEventsText}

ПРИМЕРЫ ПРАВИЛЬНЫХ СОБЫТИЙ:

НЕГАТИВНОЕ (нападение):
"На группу напали одичавшие собаки. Отбились, но Алексей неудачно упал, спасаясь от них."

Последствия:
- Алексей: перелом руки (средняя)
- Бункер: минус 1 месяц еды (собаки украли часть припасов)

НЕГАТИВНОЕ (несчастный случай):
"При разведке заброшенного дома обрушилась лестница. Мария сорвалась вниз."

Последствия:
- Мария: сотрясение мозга (легкая)
- Инвентарь Марии: потерян фонарик

ПОЗИТИВНОЕ (находка):
"В подвале разрушенного магазина нашли ящик с консервами и охотничий нож."

Последствия:
- Бункер: +4 месяца еды
- Иван: получает охотничий нож

ПОЗИТИВНОЕ (инструменты):
"Обнаружили брошенный военный грузовик. В кабине лежал ящик с инструментами."

Последствия:
- Бункер: получает бензопилу и топор
- Бункер: +2 месяца еды (нашли сухпайки)

${extraUniqInstructions}

НАПИШИ ОДНО СОБЫТИЕ (${eventType}) В ТАКОМ ФОРМАТЕ:

[Короткое описание события, 2-4 предложения]

Последствия:
- [Имя игрока или Бункер]: [конкретное изменение]`;
}
...
async function generateUniqueEvent(game, options = {}) {
  const {
    ...
    modelTemperature = 1, // если хочешь - можно прокинуть в callOpenRouterWithTimeout, см. ниже
  } = options;
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    const category = pickCategoryAvoiding(usedCategories);
    usedCategories.add(category);

    // Чем дальше попытка — тем жестче просим "перепиши полностью"
    const extra =
      attempt === 1
        ? ""
        : `ПОПЫТКА #${attempt}: Предыдущий вариант оказался слишком похож.
ПОЛНОСТЬЮ ПЕРЕПРИДУМАЙ событие: другая причина, другая находка/угроза, другой ход действий и другой итог.
Запрещено повторять структуру "заметили X -> подошли -> встретили Y -> получили Z" если она была ранее.`;

    const prompt = generateEventPromptWithCategory(game, category, extra);

    // Генерация
    const raw = await generateEventWithFallback(prompt);

    // Чистка/валидация твоей функцией
    const cleaned = validateAndCleanEvent(raw, game);

    // Проверка похожести
    const report = isTooSimilarToRecent(cleaned, game, {
      recentCount,
    ...
    if (!report.tooSimilar) {
      ...
      return {
        ...
        meta: {
          ...
          category,
          similarityMax: report.maxSim
        }
      };
    }

    console.log(
      `⚠️ Событие слишком похоже (sim=${report.maxSim.toFixed(2)}), попытка ${attempt}/${maxAttempts}, регенерируем...`
    );
  }

  // Если не смогли добиться уникальности — вернём лучший последний вариант, но пометим
  return {
    text: lastCandidate,
    meta: {
      attemptsUsed: maxAttempts,
      category: null,
      similarityMax: lastReport?.maxSim ?? null,
      warning: "Не удалось получить достаточно уникальное событие по порогу, возвращён последний вариант."
    }
  };
}
...
app.post('/api/generate-event', async (req, res) => {
  try {
    ...
    const result = await generateUniqueEvent(game, {
      maxAttempts: 6,
      recentCount: 10,
      similarityThreshold: 0.35
    });

    // Валидируем и очищаем событие
    const cleanedEvent = validateAndCleanEvent(result.text, game);
    ...
    // Применяем последствия события к игре
    applyEventConsequences(cleanedEvent, game);
