...
function generateEventPromptWithCategory(game, category, extraUniqInstructions = "") {
  ...
  if (recentEvents.length > 0) {
    recentEventsText = '⚠️ ПРЕДЫДУЩИЕ СОБЫТИЯ (НЕ ПОВТОРЯЙ ЭТО ГОВНО):\n';
    recentEvents.forEach((e, i) => {
      const t = (e.text || "").replace(/\s+/g, " ").trim();
      const eventPreview = t.substring(0, 150) + (t.length > 150 ? '...' : '');
    ...
    recentEventsText = 'Первое событие. Сделай пиздато.';
  }

  let playersList = '';
  activePlayers.forEach(p => {
    playersList += `- ${p.name} (этот долбоеб еще жив)\n`;
  });
  ...
  return `ТЫ ГЕНЕРАТОР СОБЫТИЙ ДЛЯ ИГРЫ "БУНКЕР". ТВОЙ ЮМОР - ГРУБЫЙ, ЧЕРНЫЙ, ЦИНИЧНЫЙ, КАК БУДТО ТЫ ПЬЯНЫЙ САНТЕХНИК РАССКАЗЫВАЕШЬ БАЙКИ В ЗАБРОШЕННОМ ПОДВАЛЕ.

ПРИДУМАЙ САМ ГРУБУЮ ЧЕРНУЮ ШУТКУ В НАЧАЛЕ, ЧТОБЫ ЗАДАТЬ НАСТРОЕНИЕ. ЧЕМ ПОШЛЕЕ И ЦИНИЧНЕЕ - ТЕМ ЛУЧШЕ.

ПРЕДЫДУЩЕЕ ГОВНО (НЕ ПОВТОРЯЙ ЭТУ ХЕРНЮ):
${recentEventsText}

ПРАВИЛА ФОРМАТИРОВАНИЯ (СУКА, СЛЕДУЙ ИМ):
1. Текст события — МИНИМУМ 5 ПРЕДЛОЖЕНИЙ, макс 7
2. Каждое предложение с новой строки
3. НЕ СТАВЬ кавычки, звездочки, нумерацию в начале текста
4. После текста пустая строка
5. Затем "Последствия:" с новой строки
6. Каждое последствие с новой строки, с дефиса и пробела (- )
7. Каждое последствие заканчивается <br>
8. Между последствиями НЕ СТАВЬ пустые строки
...
- Обнаружение схрона
- Встреча с выжившими
- Находка инструментов

Последствия позитивного события:
- Добавление предметов
- Увеличение запасов бункера (+X месяцев)
- Находка инструментов

ПРИМЕР ТВОЕГО ЮМОРА (ориентируйся на это):
"В новом мире даже суицид стал командным видом спорта."
"Апокалипсис — это когда твой психотерапевт просит у тебя валидол."
"Знаешь, что лучше всего растет на радиоактивной почве? Похуй, главное что растет."
"Постапокалипсис: единственное время, когда член в розетку совать — не самая тупая идея."
"Говорят, в конце света все увидят свою жизнь перед глазами. Спойлер: там тоже было не очень."
...
[Твоя грубая черная шутка, задающая настроение]
[Предложение 1 события]
[Предложение 2 события]
[Предложение 3 события]
[Предложение 4 события]
[Предложение 5 события]

Последствия:
- [Имя]: [что случилось]<br>
- [Имя/Бункер]: [что еще произошло]<br>`;
}
