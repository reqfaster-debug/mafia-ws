...
function generateEventPromptWithCategory(game, category, extraUniqInstructions = "") {
  ...
  return `Ты генератор событий для игры "Бункер". Пиши коротко, сухо, только факты. Без литературных описаний, метафор и речевых оборотов.

ПРЕДЫДУЩИЕ СОБЫТИЯ:
${recentEventsText}

ПРАВИЛА ФОРМАТИРОВАНИЯ:
1. Текст события — 3-5 коротких предложений
...
7. **ВАЖНО: Каждое последствие должно заканчиваться <br>, включая ПЕРВОЕ**
8. Между последствиями НЕТ пустых строк

ПРИМЕРЫ ПРАВИЛЬНЫХ СОБЫТИЙ:

НЕГАТИВНОЕ:
На группу напал бродячий мародер.
Отбились, но Алексей получил удар ножом.
Мародер сбежал, прихватив рюкзак с припасами.

Последствия:
- Алексей: колотая рана (средняя)<br>
- Бункер: минус 2 месяца еды<br>

ПОЗИТИВНОЕ:
В километре от бункера прошел ураган.
При осмотре местности нашли разбросанные вещи.
Обнаружили ящик с инструментами и медикаменты.

Последствия:
- Бункер: +3 месяца еды<br>
- Бункер: аптечка, набор инструментов<br>

НЕГАТИВНОЕ:
При разведке заброшенного дома обрушилась лестница.
Мария упала с высоты второго этажа.
При падении сломала руку.

Последствия:
- Мария: перелом руки (средний)<br>

ПОЗИТИВНОЕ:
Нашли труп сталкера в лесу.
В рюкзаке обнаружили консервы и охотничий нож.
Похоронили труп, забрали вещи.

Последствия:
- Бункер: +2 месяца еды<br>
- Иван: охотничий нож<br>

ТИП СОБЫТИЯ: ${eventType}

ЕСЛИ НЕГАТИВНОЕ:
- Нападение, несчастный случай, природная опасность, болезнь, поломка
Последствия: травмы/болезни, потеря предметов, уменьшение запасов, поломка

ЕСЛИ ПОЗИТИВНОЕ:
- Находка припасов, схрон, обмен с выжившими, находка инструментов
