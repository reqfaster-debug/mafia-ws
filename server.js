...
- Травма: ${facts.injury}`;
  ...
  const prompt = `Ты генератор историй для игры "Бункер". Напиши историю события (5-6 предложений) на основе фактов.

**КРИТИЧЕСКИ ВАЖНЫЕ ПРАВИЛА (НАРУШЕНИЕ НЕДОПУСТИМО):**

1. **МЕСТО ДЕЙСТВИЯ - ВСЕГДА У ВХОДА В БУНКЕР**
   - Группа не разбивает лагерь, не строит жилье, не сидит у костра
   - Игроки всегда находятся рядом со входом в бункер
   - Если игрок отходит - он отходит от входа, но всегда возвращается к нему
   - Нельзя писать про "стоянку", "лагерь", "жилище" - только "вход в бункер"

2. **ПРЕДМЕТЫ МОЖНО УПОМИНАТЬ ТОЛЬКО ЕСЛИ ОНИ ЕСТЬ В ФАКТАХ ИЛИ РАСКРЫТОМ ИНВЕНТАРЕ**
   ...
   - НЕЛЬЗЯ писать, что игрок "сел на велосипед", если велосипеда нет в инвентаре
   - НЕЛЬЗЯ писать, что игрок "достал нож", если ножа нет в инвентаре или фактах

3. **БОЛЕЗНИ И ТРАВМЫ ДОЛЖНЫ БЫТЬ ЯВНО УКАЗАНЫ В ТЕКСТЕ**
...
4. **НАХОДКА ЕДЫ ДОЛЖНА БЫТЬ ЯВНО УКАЗАНА С КОЛИЧЕСТВОМ МЕСЯЦЕВ**
...
5. **ИГРОК ВСЕГДА ВОЗВРАЩАЕТСЯ К ГРУППЕ**
   - Игрок не может умереть, потеряться, исчезнуть
   - Даже с тяжелыми травмами он возвращается
   - Итог всегда: "вернулся к группе у входа в бункер" (или "остался с группой у входа", если событие пришло к ним)

6. **ТОЛЬКО ВНЕШНИЕ ФАКТОРЫ (90%)**
   - Нападение (животные, люди, мутанты) - могут напасть на игрока, где бы он ни был
   - Природные явления (ураган, обвал, падение дерева, молния, наводнение)
   - Несчастные случаи (падение, порез, ожог, утопление)
   - Находки (только если в фактах есть предмет для находки)
   - **Встреча с другими выжившими** - они могут подойти ко входу или игрок может встретить их недалеко

7. **СТРУКТУРА РАССКАЗА (ДВА ВАРИАНТА):**

   **ВАРИАНТ А (более редкий - 10%) - ИГРОК ИДЕТ К СОБЫТИЮ:**
   1. Начало: кто, куда и зачем отошел от входа в бункер
   2. Событие: что случилось
   3. Итог: вернулся к группе у входа с [травма/находка]

   **ВАРИАНТ Б (чаще всего - 90%) - СОБЫТИЕ ПРИХОДИТ К ИГРОКУ:**
   1. Событие: кто/что появилось у входа или рядом и что произошло
   2. Итог: остался с группой у входа, но с [травма/потеря/находка]

8. **ЗАПРЕЩЕНО:**
   - Пафосные фразы про "судьбу", "надежду", "тьму", "напряжение"
   - Описание мыслей и переживаний
   ...
   - Описание лагеря, стоянки, жилища - только вход в бункер

${contextText}

${factsDescription}

**ПРИМЕРЫ ПРАВИЛЬНЫХ СОБЫТИЙ (обрати внимание на формат болезней и еды):**

**ВАРИАНТ А - Игрок отходит от входа:**
Алексей отошел от входа в бункер к заброшенной ферме, чтобы найти припасы. Он долго бродил по развалинам, пока не наткнулся на погреб, прикрытый ржавым листом железа. Спустившись внутрь, Алексей обнаружил три ящика с тушенкой (+3 месяца к запасам еды). Когда он уже выбирался наружу, прогнившая лестница подломилась, и он упал, получив ушиб спины (тяжелый). С трудом выбравшись из погреба, Алексей прихватил один ящик и, хромая, вернулся ко входу в бункер.

Последствия:
- Алексей: ушиб спины (тяжелый)<br>
- Бункер: +3 месяца еды<br>

**ВАРИАНТ Б - Природное явление у входа:**
Алексей и Мария осматривали окрестности недалеко от входа в бункер. Неожиданно налетел шквалистый ветер, и огромная ветка рухнула прямо на Алексея, придавив ему ногу. Мария с трудом оттащила ветку и помогла Алексею добраться до входа. У Алексея диагностировали перелом голени (средний).

Последствия:
- Алексей: перелом голени (средний)<br>

**ВАРИАНТ Б - Нападение у входа:**
Дима стоял недалеко от входа в бункер. Из-за скал неожиданно вышли трое вооруженных бандитов и потребовали еду. Один из бандитов ударил Диму прикладом по голове, и Дима получил сотрясение мозга (среднее). Бандиты забрали у Дмитрия ящик тушенки (+3 месяца).

Последствия:
- Дима: сотрясение мозга (среднее)<br>
- Дима: украден инвентарь<br>

**ВАРИАНТ Б - Находка у входа:**
Алексей заметил под камнями у входа в бункер край рюкзака, наполовину присыпанный землей. Раскапывая его, Алексей порезал руку об острый камень, получив резаную рану ладони (легкая). В рюкзаке оказались банки тушенки (+2 месяца к запасам еды) и запасная рация.

Последствия:
- Алексей: резаная рана (легкая)<br>
- Бункер: +2 месяца еды, рация<br>

**ВАРИАНТ Б - Внезапная атака:**
Из кустов недалеко от входа в бункер неожиданно вылетела стрела и попала Алексею в плечо. Дима сразу рванул в кусты, но нападавшие уже скрылись. Алексею извлекли стрелу, рана оказалась колотой (средняя).

Последствия:
- Алексей: колотая рана плеча (средняя)<br>



Напиши историю, используя факты. Выбери подходящий вариант. Не добавляй новых фактов. Строго следуй формату болезней (с указанием тяжести в скобках) и еды (с указанием месяцев в скобках). Все действия происходят у входа в бункер или в непосредственной близости от него.`;
