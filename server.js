...
function generateEventPrompt(game) {
  ...
  const baseCategories = [
    ...
    { text: "Атмосферные явления: полярное сияние, огни святого Эльма, миражи, гало, паргелий (ложное солнце)", themes: ['погода', 'атмосфера'] },
    { text: "Электрические явления: шаровая молния, статическое электричество, разряды, искры, короткое замыкание", themes: ['погода', 'электричество'] },
    { text: "Оптические иллюзии: световые столбы, радуга, туманные духи, брокенский призрак", themes: ['погода', 'оптика'] },
    { text: "Мистика: призраки, полтергейст, барабашка, домовой, кикимора, леший, водяной", themes: ['мистика', 'суеверия'] },
    { text: "Суеверия: приметы, гадания, заклинания, обереги, сглаз, порча, проклятия", themes: ['мистика', 'суеверия'] },
    { text: "Религиозный фанатизм: секты, жертвоприношения, культы, идолы, иконы, молитвы", themes: ['религия', 'культы'] },
    { text: "Археологические находки: раскопки, древние захоронения, скелеты, амфоры, монеты, украшения", themes: ['археология', 'история'] },
    { text: "Исторические артефакты: мечи, доспехи, каски, знамена, награды, документы", themes: ['археология', 'история'] },
    { text: "Музыкальные инструменты: гитара, гармонь, баян, скрипка, барабан, труба, пианино", themes: ['музыка', 'культура'] },
    { text: "Аудиозаписи: пластинки, кассеты, бобины, диски, плееры, магнитофоны", themes: ['музыка', 'техника'] },
    { text: "Фотоархивы: старые фотографии, альбомы, пленки, негативы, фотоаппараты", themes: ['фото', 'память'] },
    { text: "Видеозаписи: кассеты VHS, бобины, диски, камеры, видеорегистраторы", themes: ['видео', 'техника'] },
    { text: "Спортивный инвентарь: мячи, клюшки, коньки, лыжи, санки, велосипеды, гантели", themes: ['спорт', 'инвентарь'] },
    { text: "Спортивные сооружения: стадион, корт, каток, бассейн, тир, скалодром", themes: ['спорт', 'строения'] }
  ];
  ...
  if (bunkerExtra.includes('вентиляц') && !bannedThemes.has('воздух')) {
    baseCategories.push(
      ...
      { text: "Воздух: чистый, грязный, спертый, отравленный, свежий", themes: ['воздух', 'вентиляция'] }
    );
  }
  ...
  if (bunkerExtra.includes('медицин') && !bannedThemes.has('медицина')) {
    baseCategories.push(
      ...
      { text: "Операции: хирургия, ампутация, перевязка, наркоз, швы", themes: ['медицина', 'хирургия'] }
    );
  }
  ...
  if (bunkerExtra.includes('библиотек') && !bannedThemes.has('книги')) {
    baseCategories.push(
      ...
      { text: "Образование: обучение, уроки, лекции, экзамены, дипломы", themes: ['образование', 'книги'] }
    );
  }
  
  if (bunkerExtra.includes('тренажер') || bunkerExtra.includes('спорт')) {
    if (!bannedThemes.has('спорт')) {
      baseCategories.push(
        { text: "Спортивный инвентарь: мячи, клюшки, коньки, лыжи, санки, велосипеды, гантели", themes: ['спорт', 'инвентарь'] },
        { text: "Спортивные сооружения: стадион, корт, каток, бассейн, тир, скалодром", themes: ['спорт', 'строения'] },
        { text: "Травмы: растяжения, вывихи, переломы, ушибы, синяки", themes: ['спорт', 'травмы'] }
      );
    }
  }
  
  if (bunkerExtra.includes('интернет') && !bannedThemes.has('связь')) {
    baseCategories.push(
      { text: "Связь: интернет, сеть, Wi-Fi, роутеры, кабели, серверы, данные", themes: ['связь', 'интернет'] },
      { text: "Информация: новости, сообщения, соцсети, форумы, чаты", themes: ['связь', 'информация'] }
    );
  }
  
  if (bunkerExtra.includes('топлив') && !bannedThemes.has('топливо')) {
    baseCategories.push(
      { text: "Топливо: бензин, солярка, керосин, уголь, дрова, торф, нефть", themes: ['топливо', 'энергия'] },
      { text: "Транспорт: машины, мотоциклы, вездеходы, генераторы, двигатели", themes: ['транспорт', 'техника'] }
    );
  }
  ...
  if ((bunkerExtra.includes('еда') || bunkerExtra.includes('продовольств')) && !bannedThemes.has('еда')) {
    baseCategories.push(
      ...
      { text: "Порча продуктов: гниль, плесень, крысы, насекомые, воры", themes: ['еда', 'порча'] }
    );
  }
  
  // Фильтруем базовые категории, исключая темы, которые уже были
  const filteredBaseCategories = baseCategories.filter(cat => {
    // Проверяем, нет ли пересечения с bannedThemes
...
2. Событие всегда вызвано ВНЕШНИМ фактором (строго из рекомендованных категорий выше).
   Категории уже подобраны под катастрофу и бункер — используй их!
3. Событие должно быть связано с:
   - Текущей катастрофой: ${game.disaster}
   - Особенностями бункера: ${game.bunker.extra}

4. Последствия должны быть КОНКРЕТНЫМИ:
   - Здоровье: ухудшение/улучшение конкретной болезни
   - Инвентарь: добавление/потеря конкретного предмета
   - Еда: +/- месяцы (макс +2 года, -1 год)
   - Бункер: изменение характеристик

5. ОБЯЗАТЕЛЬНО должен быть неловкий/позорный момент для одного из игроков.
...
<br> - Инвентарь: добавлен/потерян предмет
<br> - Запас еды: +X месяцев / -X месяцев
<br> - Бункер: изменение (если есть)

🔴 ВАЖНО: Сгенерируй событие, которое логически связано с катастрофой "${game.disaster}" и бункером "${game.bunker.extra}". Используй категории выше!
🔴 НЕ ИСПОЛЬЗУЙ запрещенные темы и ключевые слова!`;
